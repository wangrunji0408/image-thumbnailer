...
public class JpegReader: ImageReader {
    ...
    private func parseIFD(
        ...
        for _ in 0..<entryCount {
            ...
            if isThumbnail {
                ...
                case 0x8825:  // GPS IFD
                    if let gpsLocation = try await parseGPSIFD(exifOffset: exifOffset, gpsIFDOffset: UInt64(value)) {
    ...
    private func parseGPSIFD(exifOffset: UInt64, gpsIFDOffset: UInt64) async throws -> GPSLocation? {
        ...
        for i in 0..<Int(entryCount) {
            ...
            case 0x0002:  // GPSLatitude (degrees, minutes, seconds)
                if type == 5, count == 3 {  // 3 RATIONAL values
                    latitudeData = try? await parseRationalArray(
                        at: exifOffset + UInt64(valueOffset), count: 3)
                }
            ...
            case 0x0006:  // GPSAltitude
                if type == 5 {  // RATIONAL
                    if let rational = try? await parseRational(at: exifOffset + UInt64(valueOffset)) {
                        altitude = rational
                        if altitudeRef == 1 {
        ...
        logger.debug(
            "Parsed GPS location: lat=\(latitude), lon=\(longitude), alt=\(altitude ?? 0)")

        return GPSLocation(latitude: latitude, longitude: longitude, altitude: altitude)
    }

    private func parseRational(at offset: UInt64) async throws -> Double {
    ...
    private func parseRationalArray(at offset: UInt64, count: Int) async throws -> [Double] {
        var result: [Double] = []
        for i in 0..<count {
            let rational = try await parseRational(at: offset + UInt64(i * 8))
            result.append(rational)
        }
        return result
    }

    private func convertToDecimalDegrees(
        degrees: Double, minutes: Double, seconds: Double, ref: String
    ) -> Double {
